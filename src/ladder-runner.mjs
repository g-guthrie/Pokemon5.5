...
export async function runLadderBatch(options = {}) {
  ...
  const bestOf = normalizeBestOf(options.bestOf ?? process.env.BEST_OF);
  const battleCount = bestOf || clampNumber(options.battleCount ?? process.env.BATTLE_COUNT, 1, 10000, 2);
  ...
  for (let index = 0; index < battleCount; index += 1) {
    ...
    if (run.validBenchmark) {
      ...
      summary.totals.invalidBenchmarks += 1;
      summary.seriesValid = false;
    }
    ...
    if (winsNeeded && summary.seriesValid) {
      if (summary.totals.agentAWins >= winsNeeded) {
        summary.seriesWinner = 'a';
        break;
      }
      if (summary.totals.agentBWins >= winsNeeded) {
        summary.seriesWinner = 'b';
        break;
      }
    }
  }
...
function winnerToSide(winner = '') {
  if (winner === 'Benchmark P1') return 'p1';
  if (winner === 'Benchmark P2') return 'p2';
  return null;
}

function seedForBattle(index, seedBase = 550000) {
