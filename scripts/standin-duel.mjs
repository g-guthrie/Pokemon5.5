import path from 'node:path';
import {fileURLToPath} from 'node:url';
import {runWebSocketMatch} from '../src/match-runner.mjs';

const rootDir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const outputPath = process.env.BENCH_OUT || path.join(rootDir, 'artifacts', 'standin-duel-latest.json');

const run = await runWebSocketMatch({
  outputPath,
  agents: {
    p1: {provider: 'standin', name: 'standin-p1'},
    p2: {provider: 'standin', name: 'standin-p2'},
  },
  maxTurns: Number(process.env.MAX_TURNS || 40),
  moveDelayMs: Number(process.env.MOVE_DELAY_MS || 200),
  allowFallback: false,
});

console.log(JSON.stringify(run.result, null, 2));
console.log(`wrote ${outputPath}`);
